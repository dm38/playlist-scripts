#!/usr/bin/perl -w
# Сортировка плейлиста по приоритетам, алфавиту и группам. А так же сопоставление с программой в формате xmltv
# ./sortm3u <playlist.m3u> <xmltv.xml>
use utf8;
use open ':utf8', ':std';
...
#    if ($key eq "tvg-name") {
#	if ($XMLTV{gen_channel_str_id($hash->{$key})}) {
#	    $st=$st." $key=\"$XMLTV{gen_channel_str_id($hash->{$key})}{'name'}\"";
#	    next;
#	}
#	else
#	{
...
#////////////////////////////////////////////////////////////////////
	    if (!($PLAY_LIST{$prog_num}{'params'}{'tvg-name'} &&
		  $XMLTV{gen_channel_str_id($PLAY_LIST{$prog_num}{'params'}{'tvg-name'})} &&
		  ($XMLTV{gen_channel_str_id($PLAY_LIST{$prog_num}{'params'}{'tvg-name'})}{'id'} eq $PLAY_LIST{$prog_num}{'params'}{'tvg-id'}))) {
		$channel_str_id=find_xml_prog_id($channel_name, \%XMLTV);
		if (defined($XMLTV{$channel_str_id})) {
		    $PLAY_LIST{$prog_num}{'params'}{'tvg-name'} = $XMLTV{$channel_str_id}{'name'};
		    ...
		    delete $PLAY_LIST{$prog_num}{'params'}{'tvg-name'};
